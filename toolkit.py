...
def import_ex(TexFolder,BankJSON,mode = 'append'):
    ...
    # print('  Done, {:d} files imported'.format(len(files)))
    ...
    for outName in outNames:
        ...
        for run in range(2):
            try:
                ...
                print('  - Try to compile the text file manually to find')
...
def build_xml(BankJSON,build_file):
    ...
    # Pop the meta-data out of the dictionary, configure runs.
    if 'marks' in setup:
        setup.pop('marks')               # Mark display irrelevant for XML
    if 'newPage' in setup:
    ...
    for exList in exListBase:
        for exLabel in exList:
            ...
            # marks = re.split('/',Q_content['marks'])
            ...
            for item in S_content['items']:
                ...
                if item in paramDict.keys():
                    ...
                    # SolStr = re.sub('\$','',prompt + eqsign + itemStr)
                    # SolStr = re.sub('\$','',xmlPrompts.pop(0) + '=' + itemStr)
                ...
                # Else, parse solutions from Latex
                    ...
                    # SolStr = re.sub('\$','',xmlPrompts.pop(0) + '=' + item)
                    # SolStr = xmlParse('$'+ SolStr +'$')
                    # SolStr = re.sub('<br>','',SolStr)     # Force inline 
                    # SolStrs.append(SolStr[0])
            ...
            # if 'feedback' in S_content: # AND flag is active
            ...
            for item in FdBck:
                ...
                # Check if parametric solution already exists, substitute if so
